...
class RectifiedConservativeFlowQuantile(FlowMatchingQuantile):
    def __init__(
        self,
        ...
        self.model_information_dict = {
            "class_name": "RectifiedConservativeFlowQuantile",
        }
    ...
    def predict_vector_field(
        ...
        conservative_vector_field_prediction = torch.autograd.grad(
            self.vector_field_network(
                condition=torch.cat([condition, time_embedding], dim=-1),
                tensor=state,
            ).sum(), state, create_graph=create_graph
        )[0]
        return conservative_vector_field_prediction
    ...
    def fit(
        ...
        **kwargs,
        ...
        train_parameters.number_of_epochs_to_train = max(
            1, train_parameters.number_of_epochs_to_train // 2
        )
